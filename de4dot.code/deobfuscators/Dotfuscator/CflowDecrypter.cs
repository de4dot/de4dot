...
namespace de4dot.code.deobfuscators.Dotfuscator {
    ...
    class CflowDecrypter {
        ...
        public void Descrypt(MethodDef method)
        {
            if (! method.HasBody) return;
            if (!method.Body.HasInstructions) return;
            if (method.Body.Instructions.Count < 4) return;
            if (method.Body.Variables.Count == 0) return;
            var ins = method.Body.Instructions;
            for (int i = 3; i < ins.Count; i++)
            {
                if (ins[i].OpCode.Code != Code.Ldind_I4) continue;
                if (!ins[i - 1].IsLdloc() && ins[i - 1].OpCode.Code != Code.Ldloca && ins[i - 1].OpCode.Code != Code.Ldloca_S) continue;
                if (!ins[i - 2].IsStloc()) continue;
                if (!ins[i - 3].IsLdcI4()) continue;
                if (ins[i - 1].Operand != ins[i - 2].Operand) continue;
